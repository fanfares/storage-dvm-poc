...
import {getPublicKey, tryJson, NostrConnectHandler, Session} from "./misc"
...
export class NostrConnectBroker extends Emitter {
  ...
  subscribe() {
    if (!this.handler.relays) { logger.error('unexpected: undefined this.handler.relays'); return }
    this.#sub = subscribe({
  ...
  async request(method: string, params: string[], admin = false) {
    ...
    if (!pubkey) { logger.error('unexpected: undefined pubkey'); return }
...
export class Connect {
  ...
  constructor(readonly session: Session) {
    if (this.isEnabled()) {
      ...
      if (!connectKey) { logger.error('unexpected: undefined connectKey'); return }
      if (!connectHandler) { logger.error('unexpected: undefined connectHandler'); return }
